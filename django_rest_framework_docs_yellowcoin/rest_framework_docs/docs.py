...
class DocumentationGenerator():
	...
	def __process_urlpatterns(self):
		...
		for endpoint in self.urlpatterns:
			...
			if doc.description in visited:
				continue
			visited.add(doc.description)
			doc.params = docstring_meta['params']
			doc.returns = docstring_meta['returns']
			...
			for method, items in (doc.params.items() + doc.params.items()):
				if len(items) > 0 and method not in doc.allowed_methods:
					doc.allowed_methods.append(method)
	...
	def __parse_docstring__(self, docstring):
		def parse(line, token, target):
			needle = line.find(token)
			if needle != -1:
				params = line.split(token)
				example = params[1].split('e.g.')
				params[0] = params[0].split(' ')
				methods = []
				while len(params[0]) > 0 and params[0][0] in ('GET', 'POST', 'PUT', 'PATCH', 'DELETE'):
					methods.append(params[0].pop(0))
				for method in methods:
					if len(example) > 1:
						target[method].append({'key':' '.join(params[0]).strip(), 'value':example[0].strip(), 'example':example[1].strip()})
					else:
						target[method].append({'key':' '.join(params[0]).strip(), 'value':example[0].strip(), 'example':None})
		...
		_params = {'GET':[],'POST':[],'PUT':[],'PATCH':[],'DELETE':[]}
		_returns = {'GET':[],'POST':[],'PUT':[],'PATCH':[],'DELETE':[]}
		description = ''

		for line in split_lines:
			if parse(line, '->', _params):
				continue
			if parse(line, '<-', _returns):
				continue
			description = description + line + '<br/>'
		return {'description': description, 'params': _params, 'returns': _returns}
